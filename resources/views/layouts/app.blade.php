    ...
    <div class="flex flex-1 relative overflow-hidden">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" 
            ...
            <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
                ...
                {{-- Sensus Bulanan --}}
                @can('manage-sensus')
                    <a href="{{ route('kepengasuhan.sensus') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 hover:text-white transition-all {{ request()->routeIs('kepengasuhan.sensus') ? 'bg-slate-800 text-white font-medium' : '' }}">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/></svg>
                        <span>Sensus Lama</span>
                    </a>
                ...
                @if(auth()->user()->hasRole('super-admin') || auth()->user()->hasRole('manajemen') || auth()->user()->hasRole('ketua-madrasah'))
                    <div class="pt-4 pb-2 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Madrasah</div>
                    <a href="{{ route('madrasah.kelas') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 hover:text-white transition-all {{ request()->routeIs('madrasah.kelas*') ? 'bg-slate-800 text-white font-medium' : '' }}">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                ...
                @if(auth()->user()->hasRole('super-admin') || auth()->user()->hasRole('manajemen') || auth()->user()->can('manage-roles'))
                    ...
                    <a href="{{ route('system.santri.import') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 hover:text-white transition-all {{ request()->routeIs('system.santri.import') ? 'bg-slate-800 text-white font-medium' : '' }}">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                        <span>Setup Santri & Wali (Excel)</span>
                    </a>
