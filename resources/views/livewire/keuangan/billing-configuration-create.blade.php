<div class="space-y-0">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <div class="flex items-center gap-2 text-[10px] text-slate-400 dark:text-slate-500 font-semibold mb-0.5">
                    ...
                    <span>/</span>
                    <span class="text-slate-600 dark:text-slate-300">Tambah Iuran Baru</span>
                </div>
                <h1 class="text-lg font-extrabold text-slate-900 dark:text-white tracking-tight">Buat Konfigurasi Iuran Baru</h1>
            </div>
        </div>
        <span class="hidden sm:block text-[10px] text-slate-400 font-medium bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-full">
            Tarif aktif setelah disimpan
        </span>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="lg:col-span-1 space-y-4 lg:sticky lg:top-6">
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
                <div class="px-4 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 flex items-center justify-between">
                    <div>
                        <span class="block text-[9px] font-extrabold text-emerald-100 uppercase tracking-widest">Preview Kartu Tagihan</span>
                ...
                <div class="p-4 space-y-3">
                    ...
                    <div class="text-center py-3 bg-slate-50 dark:bg-slate-950/50 rounded-xl border border-slate-100 dark:border-slate-800">
                        <span class="block text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1">Nominal Tagihan</span>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div class="p-2 bg-slate-50 dark:bg-slate-950/50 rounded-xl border border-slate-100 dark:border-slate-800 text-center">
                            ...
                            <span class="text-[10px] font-extrabold text-slate-700 dark:text-slate-300">
                                @php
                                    $intervalMap = [
                                        'monthly' => 'Bulanan (12x Dalam Setahun)',
                                        'semester' => '2x Dalam Setahun (Semesteran)',
                                        'caturwulan' => '3x Dalam Setahun (Caturwulan)',
                                        '3x_yearly' => '3x Dalam Setahun (Caturwulan)',
                                        'triwulan' => '4x Dalam Setahun (Triwulan)',
                                        '4x_yearly' => '4x Dalam Setahun (Triwulan)',
                                        'bimulanan' => '6x Dalam Setahun (Dwibulanan)',
                                        '6x_yearly' => '6x Dalam Setahun (Dwibulanan)',
                                        'yearly' => 'Tahunan (1x Dalam Setahun)',
                                        'insidental' => 'Sekali Bayar / Event'
                                    ];
                                @endphp
                        ...
                        <div class="p-2 bg-slate-50 dark:bg-slate-950/50 rounded-xl border border-slate-100 dark:border-slate-800 text-center">
                            <span class="block text-[8px] font-extrabold text-slate-400 uppercase mb-0.5">Berlaku</span>
                            <span class="text-[10px] font-extrabold text-slate-700 dark:text-slate-300">
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-slate-800 rounded-2xl p-4 shadow-sm">
                ...
                <div class="mt-2.5 space-y-1">
                    @foreach([
                        ['Nama Iuran', (bool)$newConfigName],
                        ['Tipe Tagihan', (bool)$newConfigType],
                        ['Nominal Tarif', $newConfigAmount > 0],
                        ['Tanggal Berlaku', (bool)$newConfigEffectiveFrom],
                        ['Target Penerima', $newConfigTargetType === 'all' || !empty($newConfigTargetFilters)],
                    ] as [$label, $done])
                        <div class="flex items-center gap-2 text-[10px]">
                            <span class="{{ $done ? 'text-emerald-500' : 'text-slate-300 dark:text-slate-600' }}">
                                @if($done) ✓ @else ○ @endif
                            </span>
                            <span class="{{ $done ? 'text-slate-600 dark:text-slate-300 font-semibold' : 'text-slate-400 dark:text-slate-500' }}">{{ $label }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <div class="lg:col-span-2 space-y-5">
            <form wire:submit.prevent="createConfig" class="space-y-5">
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
                    <div class="flex items-center gap-3 px-5 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20">
                        <span class="w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-extrabold shrink-0 transition-all
                            {{ ($newConfigName && $newConfigType && $newConfigAmount > 0) ? 'bg-emerald-500 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400' }}">
                            @if($newConfigName && $newConfigType && $newConfigAmount > 0) ✓ @else 1 @endif
                        </span>
                        <div>
                    ...
                    <div class="p-5 space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="block text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1.5">
                                    Tipe / Kategori Tagihan <span class="text-rose-400">*</span>
                                </label>
                                <select wire:model="newConfigType"
                                ...
                                @error('newConfigType') <span class="text-[10px] text-rose-500 mt-1 block">{{ $message }}</span> @enderror
                            </div>
                        </div>
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-slate-800 rounded-2xl shadow-sm">
                    <div class="flex items-center gap-3 px-5 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20">
                        <span class="w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-extrabold shrink-0 transition-all
                            {{ ($newConfigInterval && $newConfigEffectiveFrom) ? 'bg-emerald-500 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400' }}">
                            @if($newConfigInterval && $newConfigEffectiveFrom) ✓ @else 2 @endif
                        </span>
                        <div>
                    ...
                    <div class="p-5 space-y-5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select wire:model="newConfigInterval"
                                    class="w-full bg-white dark:bg-slate-950/60 border border-slate-200 dark:border-slate-700/80 text-slate-800 dark:text-slate-200 rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition-all">
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
                    <div class="flex items-center gap-3 px-5 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20">
                        <span class="w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-extrabold shrink-0 transition-all
                            {{ ($newConfigTargetType === 'all' || !empty($newConfigTargetFilters)) ? 'bg-emerald-500 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400' }}">
                            @if($newConfigTargetType === 'all' || !empty($newConfigTargetFilters)) ✓ @else 3 @endif
                        </span>
                        <div>
                ...
                <div class="flex items-center justify-between pt-2 pb-6">
                    ...
                    <button type="submit"
                        ...
                        <span wire:loading.remove wire:target="createConfig" class="flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                            Simpan & Publikasikan Iuran
                        </span>
                        <span wire:loading wire:target="createConfig" class="flex items-center gap-2">
